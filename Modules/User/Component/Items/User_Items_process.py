...
class User_Items_process: 
    ...
    def generate_items_table(obj):
        ...
        tree = ttk.Treeview(obj.tableframe, columns = ("Product_id", "Product_name", "Description", "Category", "Price", "Stock"), height = 20)
        tree.heading("#0") 
        ...
        #create scrollbar 
        scrollbar = Scrollbar(obj.tableframe, orient = VERTICAL, command = tree.yview) 
        scrollbar.grid(row = 0, column = 7, sticky = (N, S, W, E)) 
        tree.configure(yscrollcommand = scrollbar.set)

        #add data to tree view
        for i in range(len(data)):
            tree.insert("", i, text = str(i), values = (data[i]['Product_id'], data[i]['Product_name'], data[i]['Description'], data[i]['Category'], data[i]['Price'], data[i]['Stock']))
